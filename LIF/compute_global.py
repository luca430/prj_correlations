...
def main():
    ...
    for k in [0.0, 1.0, 1.5, 2.5, 5.0]:
        ...
        input_folder1 = f"/mnt/filtered_matrices/LIF/shapes/K_{k}"
        input_folder2 = f"/mnt/filtered_matrices/LIF/spike_trains/K_{k}"
        ...
        for root, _, files in os.walk(input_folder1):
            for file_name in files:
                if file_name.endswith(".npz"):
                    ...
                    params.append([input_file_path, output_folder1, k])
        for root, _, files in os.walk(input_folder2):
            for file_name in files:
                if file_name.endswith(".npz"):
                    input_file_path = os.path.join(root, file_name)
                    params.append([input_file_path, output_folder2, k])
    ...
    with Manager() as manager:
        ...
        file_path = os.path.join(output_folder1, "original_global.npy")
        np.save(file_path, dict(N_dict))
        file_path = os.path.join(output_folder2, "original_global.npy")
        np.save(file_path, dict(N_dict))
    print('\tDone!')
